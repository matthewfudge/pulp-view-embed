// design_render — render a DesignIR JSON (incl. the faithful-vector lane:
// faithful_svg nodes → DesignFrameView with native SVG-patch knobs + native
// overlay controls) through the embed's native DesignIR path to a PNG.
//   design_render <design.ir.json> <w> <h> <out.png> [scale]
//
// Uses pulp_embed_create_from_design_json (which calls build_native_view_tree),
// attaches to an offscreen NSWindow so the GPU host path runs, then captures via
// render_png. Standalone fidelity harness (not a gate), sibling of bundle_render.
...
int main(int argc, const char* argv[]) {
    if (argc < 5) {
        std::fprintf(stderr, "usage: %s <design.ir.json> <w> <h> <out.png> [scale]\n", argv[0]);
        return 2;
    }
    ...
    @autoreleasepool {
        ...
        PulpEmbedResult r = pulp_embed_create_from_design_json(&d, json.c_str(), &v);
        if (r != PULP_EMBED_OK || !v) {
            char buf[1024];
            pulp_embed_last_create_error(buf, sizeof(buf));
            std::fprintf(stderr, "create_from_design_json failed: result=%d err=%s\n", r, buf);
            return 1;
        }
